...
public class Segment {
    ...
    private ArrayList<DustOptions> dustOptions;
    ...
    public Segment(Location startLocation, double length, int pointAmount, Color color, float startSize, float endSize) {
        this.parent = null;
        this.child = null;
        ...
        this.dustOptions = new ArrayList<DustOptions>();
        float inc = (this.endSize - this.startSize) / this.pointAmount;
        for (int i = 0; i < pointAmount; i++) {
            dustOptions.add(new DustOptions(this.color, this.startSize + (i * inc)));
        }
    ...
    public Segment(Segment parent, double length, int pointAmount, Color color, float startSize, float endSize) {
        this.parent = parent;
    ...
    private void drawLine(Location a, int pointAmount, double length) {
        ...
        for (double i = 0; i < length; i += pointIncrement) {
            a.getWorld().spawnParticle(Particle.REDSTONE, pointLocation, 1, 0, 0, 0, this.dustOptions.get(count++));
